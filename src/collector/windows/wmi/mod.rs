...
use self::values::{health_status, physical_media_type, storage_bus_type};
use super::storage::StorageDescriptor;
...
fn collect_storage_descriptors(
    ...
    indexes
        .into_iter()
        .filter_map(|index| match super::storage::query_physical_drive(index) {
            Ok(descriptor) => {
                if debug_enabled {
                    debug.push(
                        DebugRecord::new(
